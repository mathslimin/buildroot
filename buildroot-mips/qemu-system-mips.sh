...
if [[ "${ARG_DRIVER:-n}" != "n" ]]; then
	QEMU_OPT_driver="-drive file=${ARG_DRIVER},format=raw"
...
if [[ "${ARG_HOST:-n}" != "n" || "${ARG_PORT:-n}" != "n" ]]; then
	device="-device pcnet,netdev=net0"
...
QEMU_OPT_machine="-machine malta"
QEMU_OPT_cpu="-cpu mips32r6-generic"
# QEMU_OPT_smp="-smp 1"
QEMU_OPT_m="-m 1024M"
