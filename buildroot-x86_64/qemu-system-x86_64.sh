...
DEFAULT_KERNEL=${DEFAULT_IMAGES_DIR:-.}/bzImage
...
if [[ "${ARG_DRIVER:-n}" != "n" ]]; then
	QEMU_OPT_boot="-boot c"
	QEMU_OPT_driver="-hda ${ARG_DRIVER}"
	QEMU_OPT_append_root="root=/dev/sda rw"
fi
...
if [[ "${ARG_HOST:-n}" != "n" || "${ARG_PORT:-n}" != "n" ]]; then
	device="-device e1000,netdev=net0"
...
QEMU_OPT_append_console="console=ttyS0"
...
#QEMU_OPT_machine="-machine q35"
#QEMU_OPT_cpu="-cpu host -enable-kvm"
#QEMU_OPT_cpu="-cpu Cascadelake-Server"
QEMU_OPT_smp="-smp 2"
QEMU_OPT_m="-m 1024M"
...
QEMU_OPT=${QEMU_OPT}" ${QEMU_OPT_boot}"
