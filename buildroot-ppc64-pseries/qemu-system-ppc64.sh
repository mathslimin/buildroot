...
DEFAULT_KERNEL=${DEFAULT_IMAGES_DIR:-.}/vmlinux
...
if [[ -n "${ARG_DRIVER}" ]]; then
	QEMU_OPT_driver="-drive file=${ARG_DRIVER},if=scsi,index=0,format=raw"
	QEMU_OPT_append_root="root=/dev/sda rw"
fi

if [[ -n "${ARG_INITRD}" ]]; then
...
QEMU_OPT_append_console="console=hvc0"
...
QEMU_OPT_machine="-machine pseries"
QEMU_OPT_cpu="-cpu power8"
QEMU_OPT_smp="-smp 2"
QEMU_OPT_m="-m 1024M"
